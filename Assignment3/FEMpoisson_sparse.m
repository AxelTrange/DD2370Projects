...
figure;plot(xy(1:2:end),xy(2:2:end),'k')
xlabel('x'),ylabel('y');

x = 100*no2xy(1,:);
y = 100*no2xy(2,:);
%tri = delaunay(x,y); %delaunay does pretty much same thing as el2no.
tri = el2no';
figure;trisurf(tri,x,y,z)
xlabel('x'); ylabel('y'); title('\Phi - Finite Element')
